...
2. **Extraction** — a small private helper `get_order_note_from_payload( array $payload ): string` returns `wp_kses_post( trim( $payload['order_note'] ) )` for a string, otherwise `''`. It is read from the **original** payload before `prepare_payload_for_wc_order()` runs.
